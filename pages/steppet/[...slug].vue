...
const { data: article } = await useAsyncData(
  `steppet-${slug.join('/')}`,
  () => queryContent('/steppet').where({ _path: `/steppet/${slug.join('/')}` }).findOne()
)
  ...
  <section class="relative overflow-hidden pt-32 pb-24">
    ...
    <BaseContainer>
      <div class="mx-auto max-w-3xl relative">
        <NuxtLink to="/steppet" class="mb-8 inline-flex items-center gap-2 text-sm text-[#888] hover:text-[#f0f0f0] transition-colors group">
